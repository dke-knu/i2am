...
//	    	.attr("transform", "translate(-" + x(numSteps*data.step) + ")");
	...
	this.updateData = function(newData) {
	    ...
	    graph.selectAll("g .circles circle").data(data.values)
	...
	var graph, x, yLeft, yRight, xAxis, yAxisLeft, yAxisRight, yAxisLeftDomainStart, linesGroup, circlesGroup, linesGroupText, lines, circles, lineFunction, circleXFunction, circleYFunction, lineFunctionSeriesIndex = -1, circleFunctionSeriesIndex=-1;
	...
	var processDataMap = function(dataMap) {
		...
		// updated by swson.
		// add lineType.
		var lineTypes = getOptionalVar(dataMap, 'lineTypes', []);
		// default colors values
		...
		// updated by swson.
		// add lineType.
		var opacity = getOptionalVar(dataMap, 'opacity', []);
		// default colors values
		if(opacity.length == 0) {
			displayNames.forEach(function (v, i) {
				// set the default
		...
		*/
		...
		if(withTransition) {
			...
			circlesGroup.selectAll("circle")
				...
				.attr("r", 5.5)
				.attr("fill", "none")
				.attr("cx", circleXFunction)
				.attr("cy", circleYFunction)
				.attr("transform", null);
	...
	var createGraph = function() {
		...
		linesGroup.append("path")
				...
				.attr("opacity", function(d, i) {
					return data.opacity[i];
				})
				// updated by swson.
				// dashed line.
				.attr("stroke-dasharray", function(d, i) {
					if (data.lineTypes[i] == "dashed")
						return ("5", "5");
					else
						return ("0", "0");
				})
		...
		// add a line group for each array of values (it will iterate the array of arrays bound to the data function above)
		circlesGroup = circles.enter().append("g")
				.attr("class", function(d, i) {
					return "line_group series_" + i;
				})
				.attr("stroke", function(d, i) {
				...
				// circle opacity.
				.attr("opacity", function(d, i) {
					return (1-data.opacity[i]);
				});
		
		circleXFunction = function(d, i) { 
			var _x = x(data.startTime.getTime() + (data.step*i)); 
			
			return d!=null?_x:'';
		}
		
		circleYFunction = function(d, i) {
			if (this.parentNode.getAttribute("opacity")==0) {
			...
			if(i == 0) {
				circleFunctionSeriesIndex++;
			}
			var axis = data.axis[circleFunctionSeriesIndex];
			var _y;
			if(axis == 'right') { 
				_y = yRight(d); 
			} else {
		...
		// add circle (the actual line) to line group
		circlesGroup.selectAll("circle")
				.data(function (d) { return d; })
				.enter().append("circle")
				.attr("r", 5.5)
