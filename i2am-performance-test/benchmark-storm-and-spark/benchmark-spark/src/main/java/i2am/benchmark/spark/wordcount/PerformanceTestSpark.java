...
public class PerformanceTestSpark {	
	...
	public static void main(String[] args) throws InterruptedException {
		...
		kafkaParams.put("bootstrap.servers", "192.168.56.100:9092");
		...
		JavaDStream<String> wordCounts = lines.transform( line -> {
			
			// Each RDD !!
			JavaRDD<String> result = line.map( in -> {
						
				System.out.println("####### "+in);
				logger.error("@@@@@ "+in);
				
				String[] commands = in.split(","); // Split Command				
				...
				output = output + "}" + "," + commands[1] + "," + commands[2] + "," + System.currentTimeMillis();
		...
		wordCounts.foreachRDD(								
				output -> {										
					for( String tuple: output.collect() ) {							
						producer.send(new ProducerRecord<String, String>(output_topic, tuple));
					}								
				});		
