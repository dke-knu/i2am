...
public class SparkQueryTest7 {	

	private final static Logger logger = Logger.getLogger(SparkQueryTest7.class);	
	...
	public static void main(String[] args) throws InterruptedException {
		...
		// MN.eth 9092.
		String[] zookeeper_ip = args[4].split(",");
		String zookeeper_port = args[5];
		
		boolean init = true;
		String zk = null;
		
		for( String temp: zookeeper_ip ) {
			
			if ( init ) {
				zk = temp + ":" + zookeeper_port;
				init = false;
				
			} else {
				
				zk = zk + "," + temp + ":" + zookeeper_port;  
			}			
		}		
